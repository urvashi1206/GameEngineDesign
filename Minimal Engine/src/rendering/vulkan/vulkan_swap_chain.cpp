...
namespace minimal
{
    ...
    void vulkan_swap_chain::createSwapChain()
    {
        ...
        createInfo.oldSwapchain = old_swap_chain_ == nullptr ? VK_NULL_HANDLE : old_swap_chain_->swapChain;

        auto vk_create_swapchain_khr = vkCreateSwapchainKHR(device.get_device(), &createInfo, nullptr, &swapChain);
    ...
    void vulkan_swap_chain::createImageViews()
    {
        ...
        for (size_t i = 0; i < swapChainImages.size(); i++)
        {
            ...
            if (vkCreateImageView(device.get_device(), &viewInfo, nullptr, &swapChainImageViews[i]) !=
                VK_SUCCESS)
            {
    ...
    void vulkan_swap_chain::createSyncObjects()
    {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
        {
            if (vkCreateSemaphore(device.get_device(), &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) !=
                VK_SUCCESS ||
                vkCreateSemaphore(device.get_device(), &semaphoreInfo, nullptr, &renderFinishedSemaphores[i]) !=
                VK_SUCCESS ||
                vkCreateFence(device.get_device(), &fenceInfo, nullptr, &inFlightFences[i]) != VK_SUCCESS)
            {
