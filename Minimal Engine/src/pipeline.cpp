...
namespace minimal {
    ...
    std::vector<char> pipeline::read_file(const std::string &file_path) {
        ...
        if (!file.is_open())
            throw std::runtime_error("failed to open file: " + engine_dir);
    ...
    void pipeline::create_graphics_pipeline(const std::string &vert_path,
        ...
        auto binding_description = model::vertex::get_binding_descriptions();
        ...
        if (vkCreateGraphicsPipelines(device_.get_device(), VK_NULL_HANDLE, 1, &pipeline_info, nullptr, &graphics_pipeline_) != VK_SUCCESS)
