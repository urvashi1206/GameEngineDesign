...
namespace std {
    template<>
    struct hash<Minimal::Mesh::Vertex> {
        size_t operator()(const Minimal::Mesh::Vertex &v) const {
            size_t seed = 0;
...
namespace Minimal {
    ...
    Mesh::Mesh(VulkanDevice &device, const Builder &builder) : m_device{device} {
    ...
    std::unique_ptr<Mesh> Mesh::createModelFromFile(VulkanDevice &device, const std::string &filePath) {
        Builder builder{};
