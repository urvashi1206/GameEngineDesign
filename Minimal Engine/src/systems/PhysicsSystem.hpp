...
namespace Minimal {
    ...
    public:
        ...
        void update(float frameInfo) override;

    private:
        bool GJK(const TransformComponent &transformA, const ColliderComponent &a, const TransformComponent &transformB, const ColliderComponent &b,
                 std::vector<ContactPoint> &out_contactPoints);
        ...
        std::vector<ContactPoint> EPA(std::deque<glm::vec3> simplex, std::unordered_map<glm::vec3, std::pair<glm::vec3, glm::vec3> > supportPoints,
                                      const TransformComponent &transformA, const ColliderComponent &a, const TransformComponent &transformB, const ColliderComponent &b);

        void EPA_GetPolytopeNormals(std::vector<glm::vec3> polytope, std::vector<unsigned int> faces, std::vector<std::pair<glm::vec3, float> > &out_normals,
                                    unsigned int &out_minNormalIndex);
        ...
        std::vector<glm::vec3> EPA_GetAlignedFace(const TransformComponent &transform, const ColliderComponent &collider, const glm::vec3 &direction,
                                                  glm::vec3 &out_faceNormal) const;
    };
}
