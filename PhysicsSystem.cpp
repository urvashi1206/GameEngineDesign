...
void PhysicsSystem::Update(ECSCoordinator& ecs, float deltaTime)
{
	...
	while(simulationTimeLeft >= PHYSICS_TICK)
	{
		...
		for(Entity e = 0; e < MAX_ENTITIES; e++)
			if(ecs.HasComponent<Rigidbody>(e) && ecs.HasComponent<Collider>(e))
			{
				...
				RigidbodyUtils::UpdatePhysics(transform, rb, Instance->tickPhysics ? PHYSICS_TICK : 0);
			}
		...
		for(Entity e1 = 0; e1 < MAX_ENTITIES; e1++)
			for(Entity e2 = 0; e2 < MAX_ENTITIES; e2++)
			{
				...
				Collider& colliderA = ecs.GetComponent<Collider>(e1);
				Collider& colliderB = ecs.GetComponent<Collider>(e2);

				std::vector<ContactPoint> contactPoints;
				if(Instance->GJK(colliderA, colliderB, contactPoints))
				{
		...
		for(CollisionData& collisionData : collisions)
		{
			...
			Rigidbody& bodyA = ecs.GetComponent<Rigidbody>(entityA);
			Rigidbody& bodyB = ecs.GetComponent<Rigidbody>(entityB);
			const Collider& colliderA = *collisionData.colliderPair.GetFirst();
			const Collider& colliderB = *collisionData.colliderPair.GetSecond();
			Transform& transformA = ecs.GetComponent<Transform>(entityA);
			Transform& transformB = ecs.GetComponent<Transform>(entityB);

			Vector initialLocationA = colliderA.center;
			Vector initialLocationB = colliderB.center;
			Vector initialVelocityA = bodyA.velocity;
			Vector initialVelocityB = bodyB.velocity;
			Vector initialAngularVelocityA = bodyA.angularVelocity;
			Vector initialAngularVelocityB = bodyB.angularVelocity;

			float inverseMassA = RigidbodyUtils::GetInverseMass(bodyA);
			float inverseMassB = RigidbodyUtils::GetInverseMass(bodyB);
			Matrix4x4 inertiaTensorA = RigidbodyUtils::GetInertiaTensor(transformA, colliderA, bodyA);
			Matrix4x4 inertiaTensorB = RigidbodyUtils::GetInertiaTensor(transformB, colliderB, bodyB);
			...
			for(const ContactPoint& contact : collisionData.contacts)
			{
				...
				Vector rA = contact.location - colliderA.center;
				Vector rB = contact.location - colliderB.center;
				Vector relativeVelocity = bodyB.velocity + bodyB.angularVelocity.Cross(rB) - bodyA.velocity - bodyA.angularVelocity.Cross(rA);
				...
				j /= inverseMassA + inverseMassB +
					contact.normal.Dot((inertiaTensorA.Inverse() * rA.Cross(contact.normal)).Cross(rA)) +
					contact.normal.Dot((inertiaTensorB.Inverse() * rB.Cross(contact.normal)).Cross(rB));
				...
				float angularEffectFrictionA = rA_cross_t.Dot(inertiaTensorA.Inverse() * rA_cross_t);
				float angularEffectFrictionB = rB_cross_t.Dot(inertiaTensorB.Inverse() * rB_cross_t);
				...
				Vector correction = contact.normal * std::max(contact.penetrationDepth - slop, 0.0f) / (inverseMassA + inverseMassB) * percent;
...
bool PhysicsSystem::GJK(const Collider& a, const Collider& b, std::vector<ContactPoint>& out_contactPoints)
{
	...
	Vector difference = GJK_Support(a, direction) - GJK_Support(b, -direction);
	...
	while(true)
	{
		...
		Vector support = GJK_Support(a, direction) - GJK_Support(b, -direction);
		...
		supportPoints.emplace(support, std::make_pair<Vector, Vector>(GJK_Support(a, direction), GJK_Support(b, -direction)));
...
bool PhysicsSystem::UpdateSimplex(std::deque<Vector>& simplex, Vector& direction)
{
	switch(simplex.size())
	{
		case 2: // Line test
...
bool PhysicsSystem::UpdateSimplex_LineCase(std::deque<Vector>& simplex, Vector& direction)
{
	Vector a = simplex[0];
	Vector b = simplex[1];
...
bool PhysicsSystem::UpdateSimplex_TriangleCase(std::deque<Vector>& simplex, Vector& direction)
{
	Vector a = simplex[0];
	Vector b = simplex[1];
	Vector c = simplex[2];
...
bool PhysicsSystem::UpdateSimplex_TetrahedronCase(std::deque<Vector>& simplex, Vector& direction)
{
	Vector a = simplex[0];
	Vector b = simplex[1];
	Vector c = simplex[2];
...
std::vector<ContactPoint> PhysicsSystem::EPA(std::deque<Vector> simplex, std::unordered_map<Vector, std::pair<Vector, Vector>> supportPoints, const Collider& a, const Collider& b)
{
	...
	while(minDistance == FLT_MAX)
	{
		...
		Vector supportPoint = GJK_Support(a, minNormal) - GJK_Support(b, -minNormal);
		...
		if(abs(sDistance - minDistance) > EDGE_TOLERANCE)
		{
			...
			supportPoints.emplace(supportPoint, std::make_pair<Vector, Vector>(GJK_Support(a, minNormal), GJK_Support(b, -minNormal)));
	...
	Vector normalA;
	std::vector<Vector> faceA = EPA_GetAlignedFace(a, minNormal, normalA);
	Vector normalB;
	std::vector<Vector> faceB = EPA_GetAlignedFace(b, -minNormal, normalB);
...
void PhysicsSystem::EPA_GetPolytopeNormals(std::vector<Vector> polytope, std::vector<unsigned int> faces, std::vector<std::pair<Vector, float>>& out_normals, unsigned int& out_minNormalIndex)
{
...
std::vector<Vector> PhysicsSystem::GetContactPoints(std::vector<Vector> incidentFace, std::vector<Vector> referenceFace, Vector referenceNormal)
{
...
void PhysicsSystem::ClipPolygonAgainstPlane(std::vector<Vector>& points, Vector planeNormal, float planeOffset)
{
...
Vector PhysicsSystem::GJK_Support(const Collider& collider, const Vector& direction) const
{
	switch(collider.colliderType)

	return Vector();
}
std::vector<Vector> PhysicsSystem::EPA_GetAlignedFace(const Collider& collider, const Vector& direction, Vector& out_faceNormal) const
{
